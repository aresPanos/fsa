...
from dataloader.autoaugment import CIFAR10Policy, Cutout
...
class CIFAR10(VisionDataset):
    ...
    def __init__(self, root, train=True, transform=None, target_transform=None,
                 download=False, index=None, base_sess=None, autoaug=1, iscl_all=False, session=0, is_32_img_size=True, imagenet_norm=True):
        ...
        if base_sess:
            ...
            if train:
                if not iscl_all:
                    ...
                    #self.data, self.targets = self.SelectfromDefault(self.data, self.targets, np.arange(60))
                else:
                    if session != 0:
                        data_base, targets_base = self.SelectfromDefault(self.data, self.targets, np.arange(60))
                        data_txt, targets_txt = self.MyClassSelector(self.data, self.targets, index, session)

                        self.data = np.vstack((data_base, data_txt))
                        self.targets = np.hstack((targets_base, targets_txt))
                    else:
                        self.data, self.targets = self.SelectfromDefault(self.data, self.targets, np.arange(60))
    ...
    def MyClassSelector(self, data, targets, index, session):
        data_tmp = []
        targets_tmp = []
        ind_list = [int(i) for i in index]
        ind_np = np.array(ind_list)
        index = ind_np.reshape((session * 5, 5))
        for i in index:
            ind_cl = i
            if len(data_tmp) == 0:
                data_tmp = data[ind_cl]
