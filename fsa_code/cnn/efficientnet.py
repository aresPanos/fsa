...
from cnn.efficientnet_utils import (
    round_filters,
    round_repeats,
    drop_connect,
...
class EfficientNetLastLayer(nn.Module):
    """EfficientNet model.
       Most easily loaded with the .from_name or .from_pretrained methods.
        ...
        for i, module in enumerate(self.children()):
            if i < 3:
                if isinstance(module, torch.nn.ModuleList):
                    for jj in range(12):
                        for param in module[jj].parameters():
                            param.requires_grad = False
                else:
                    for param in module.parameters():
                            param.requires_grad = False

        print("27.8% of the number of total parameters are frozen.")
    ...
    def train(self, mode: bool=True):
        ...
        for i, module in enumerate(self.children()):
            if i < 3:
                if isinstance(module, torch.nn.ModuleList):
                    for jj in range(12):
                        module[jj].train(False)
                    for jj in range(12, len(module)):
                        module[jj].train(mode)
                else:
                    module.train(False)
            else:
                module.train(mode)
...
def efficientnet(type='efficientnet-b0', pretrained=True, num_classes=0):
    if pretrained:
        return EfficientNet.from_pretrained(type, num_classes=num_classes)
    else:
        return EfficientNet.from_name(type, num_classes=num_classes)

def efficientnetLastLayer(type='efficientnet-b0', num_classes=0):
    return EfficientNetLastLayer.from_pretrained(type, num_classes=num_classes)
